...
<<<<<<< HEAD
=======
 * 将 <input type="date"> 的值 (YYYY-MM-DD) 转换为 API 需要的 YYYYMMDD
 */
function formatDateForApi(dateValue, fallback = '') {
    if (!dateValue) return fallback;
    // 支持两种输入：如果用户已经输入 YYYYMMDD，则直接返回
    if (/^\d{8}$/.test(dateValue)) return dateValue;
    // 否则尝试 YYYY-MM-DD -> YYYYMMDD
...
=======
    const startDateRaw = document.getElementById('start-date-1').value.trim();
    const endDateRaw = document.getElementById('end-date-1').value.trim();
    const startDate = formatDateForApi(startDateRaw, '20210101');
    const endDate = formatDateForApi(endDateRaw, '20251231');
>>>>>>> 82a7c6e (Initial commit: project import)
...
=======
    const startDateRaw = document.getElementById('start-date-2').value.trim();
    const endDateRaw = document.getElementById('end-date-2').value.trim();
    const startDate = formatDateForApi(startDateRaw, '20250101');
    const endDate = formatDateForApi(endDateRaw, '20251231');
>>>>>>> 82a7c6e (Initial commit: project import)
...
=======
        if (stockCodes.length > 30) {
            showToast('最多支持 30 只股票，请减少输入数量', 'error');
            return;
        }
>>>>>>> 82a7c6e (Initial commit: project import)
...
=======
    const startDateRaw = document.getElementById('start-date-3').value.trim();
    const endDateRaw = document.getElementById('end-date-3').value.trim();
    const startDate = formatDateForApi(startDateRaw, '20250101');
    const endDate = formatDateForApi(endDateRaw, '20251231');
>>>>>>> 82a7c6e (Initial commit: project import)
