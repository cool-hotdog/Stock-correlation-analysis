...
=======
def _normalize_date_param(date_value: Optional[str], default: str) -> str:
    """Normalize date input to YYYYMMDD string.

    Accepts YYYY-MM-DD or YYYYMMDD (or None/empty) and returns YYYYMMDD.
    Raises ValueError if the normalized value is not a valid 8-digit date string.
    """
    if not date_value:
        return default
    ...
    if not (len(s) == 8 and s.isdigit()):
        raise ValueError(f"日期格式错误，期望 YYYYMMDD 或 YYYY-MM-DD，收到: {date_value}")
    return s
...
def get_stock_return_daily(
    ...
    end_date: Optional[str] = "20251231",
...
=======
    # 规范化日期参数，支持 YYYY-MM-DD 或 YYYYMMDD
    start_date_norm = _normalize_date_param(start_date, "20210101")
    end_date_norm = _normalize_date_param(end_date, "20251231")

    df = pro.daily(ts_code=ts_code, start_date=start_date_norm, end_date=end_date_norm)
>>>>>>> 82a7c6e (Initial commit: project import)
...
=======
    # 规范化日期参数
    start_date = _normalize_date_param(start_date, "20210101")
    end_date = _normalize_date_param(end_date, "20251231")
